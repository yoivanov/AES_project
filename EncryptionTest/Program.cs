using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

using System.IO;
using System.Text;
using System.Security.Cryptography;

//use namespace
using AesLib;




namespace EncryptionTest
{
    class Program
    {
        // declare global variable
        private static AesLib.Aes.KeySize keysize;

        static private byte[] SBox =
            {
        };


        static private byte[] invSBox =
            {
        };


        // ======================
        // HELPING METHODS
        // ======================


        // either user input or measuring the length of the given key should 
        // determine the number of rounds of the AES encryption
        //x 10, 12 or 14 times, depending on whether the key is 128, 192 or 256 - bit
        private static int CalculateNumIterations(string key)
        {

            Console.WriteLine("User enters 128/192/256 size of key or the key is measured after entering");

            return 10;
        }


        // Add round key method

        private static byte[] AddRoundKey(byte[] state, byte[] key)
        {
            Console.WriteLine("\nSTEP 1 \n");
            Console.WriteLine("\n === Add round key ===");
            Console.WriteLine("\nInitial block");
            Console.WriteLine(Encoding.UTF8.GetString(state));
            Console.WriteLine(string.Join(" ", state));
            return state;
        }

        // Substitute bytes method

        private static byte[] SubstituteBytes(byte[] state)
        {
            Console.WriteLine("\nSTEP 2 \n");
            Console.WriteLine("\n === Byte substitution ===");
            Console.WriteLine("\nInitial block");
            Visualize(state);

            for (int i = 0; i < 16; i++)
            {
                state[i] = SBox[state[i]];
            }

            Console.WriteLine("\nAfter substitution\n");
            Visualize(state);

            return state;
        }

        // Shift rows method

        private static byte[] ShiftRows(byte[] state)
        {
            Console.WriteLine("\nSTEP 3 \n");
            Console.WriteLine("\n === Shift rows ===");
            Console.WriteLine("\nInitial block");
            Visualize(state);

            byte[] tmp = new byte[16];

            tmp[0] = state[0];
            tmp[1] = state[5];
            tmp[2] = state[10];
            tmp[3] = state[15];

            tmp[4] = state[4];
            tmp[5] = state[9];
            tmp[6] = state[14];
            tmp[7] = state[3];

            tmp[8] = state[8];
            tmp[9] = state[3];
            tmp[10] = state[2];
            tmp[11] = state[7];

            tmp[12] = state[12];
            tmp[13] = state[1];
            tmp[14] = state[6];
            tmp[15] = state[11];

            Console.WriteLine("\nAfter shift rows\n");
            Visualize(tmp);

            return tmp;
        }

 

        private static Byte GMul(Byte a, Byte b)
        { // Galois Field (256) Multiplication
            Byte p = 0;
            Byte counter;
            Byte hi_bit_set;

            for (counter = 0; counter < 8; counter++)
            {
                if ((b & 1) != 0)
                {
                    p ^= a;
                }
                hi_bit_set = (Byte)(a & 0x80);

                a <<= 1;

                if (hi_bit_set != 0)
                {
                    a ^= 0x1b; /* x^8 + x^4 + x^3 + x + 1 */
                }
                b >>= 1;
            }
            return p;
        }


        


        private static void Visualize(byte[] array)
        {
            Console.WriteLine(Encoding.UTF8.GetString(array));
            Console.WriteLine(string.Join(" ", array));
        }


        // ======================
        // Main encryption method
        // ======================

        private static string AESenctypt(string text, string key)
        {

            // determine the number of rounds for the algorhythm
            int numberOfRounds = CalculateNumIterations(key);
            Console.WriteLine("Number of iterations to do is " + numberOfRounds);

            // === Key expansion ===
            // Expand the keys to round keys

            Console.WriteLine("expanding round keys");
            //byte[][] roundKeys = KeysExpand(key);


            // Splitting the message into blocks

            List<string> blocks = new List<string>();


            for (int i = 0; i < text.Length; i = i + 16)
            {
                if (text.Length - i < 16)
                {
                    //if the last block is smaller than our needed chunk size, add the needed number of spaces
                    blocks.Add(text.Substring(i) + new string(' ', 16 - (text.Length - i)));
                }
                else
                {
                    blocks.Add(text.Substring(i, 16));
                }

            }

            //
            // Loop through all 128 bit blocks of the message
            //

            foreach (string item in blocks)
            {
                // splitting string chunks into byte blocks
                byte[] block = Encoding.ASCII.GetBytes(item);

                Console.WriteLine("message block " + blocks.IndexOf(item));

                foreach (byte element in block)
                {
                    // print each byte of the block for now
                    Console.WriteLine("{0} = {1}", element, (char)element);
                }

                byte[] state = block;


                // STEP 1 - the first round key is added
                // === Add round key ===
                state = AddRoundKey(state, new byte[] { }); // add round key 0 roundKeys[0]

                // Start a loop with algorthytm number of steps
                for (int i = numberOfRounds; i > 0; i--)
                {

                    Console.WriteLine("\nIteration " + i);

                    // STEP 2
                    // === Byte substitution ===
                    state = SubstituteBytes(state);


                    // STEP 3
                    // === Shift rows ===
                    state = ShiftRows(state);


                    // STEP 4
                    // === Mix columns ===
                    // IT IS NOT DONE IN THE LAST ITERATION
                    if (i == 1)
                    {
                        Console.WriteLine("Last iteration, skipping mix columns");
                    }
                    else
                    {
                        //state = MixColumns(state);
                    }

                    // STEP 5
                    // === Add round key ===
                    state = AddRoundKey(state, new byte[] { });

                }
            }


            return "Encrypted text";
        }



         




        static void Main(string[] args)
        {
            // Starting with a console application
            string originalText = "";
            string key = "";

            Console.WriteLine("Choose Encrypt/Decrypt - 1/2");
            int mode = int.Parse(Console.ReadLine());

            if (mode == 1)
            {
                Console.WriteLine("Enter text to encrypt");
                originalText = Console.ReadLine();
                Console.WriteLine("Enter the secret key");
                key = Console.ReadLine();

                

                byte[] plainText = new byte[16];
                byte[] keybytes = new byte[16];
                byte[] cipherText = new byte[16];
                byte[] decryptedBytes = new byte[16];
                


                plainText = Encoding.UTF8.GetBytes(originalText);
                keybytes = Encoding.UTF8.GetBytes(key);

                AesLib.Aes a = new AesLib.Aes(AesLib.Aes.KeySize.Bits192, keybytes);

                byte[] enciphered = a.AESEncryptPadding(originalText);


                a.Dump();

                Console.WriteLine("With AES LIB " + a.VisualizeInMonkey(enciphered));
                Console.WriteLine("With AES LIB " + a.VisualizeInHex(enciphered));

                
                byte[] verified = ManagedAesSample.EncryptAesManaged(originalText, keybytes);

                Console.WriteLine("With AES INTERNAL " + a.VisualizeInMonkey(verified));
                Console.WriteLine("With AES INTERNAL " + a.VisualizeInHex(verified));

                Console.ReadKey();

                AesLib.Aes b = new AesLib.Aes(AesLib.Aes.KeySize.Bits192, keybytes);

                byte[] deciphered = b.Decrypt(enciphered);

                Console.WriteLine("Decyphered: " + Encoding.UTF8.GetString(deciphered));


                //a.Decrypt(cipherText, decryptedBytes);
                //a.Dump();

                //string decryption_hex = string.Join(" ", plainText.Select(x => x.ToString("X2")));
                //string decryption_clear = string.Join(" ", decryptedBytes.Select(x => x.ToString("X2")));

                //Console.WriteLine($"Decrypted in hex: {decryption_hex}");
                //Console.WriteLine($"Decrypted in plain text: {Encoding.UTF8.GetString(decryptedBytes)}");
            }
            else
            {
                Console.WriteLine("Not implemented yet");
            }
        }
    }
}
